...
export function ChatWidget({ defaultOpen = false }: ChatWidgetProps) {
  ...
  const loadOlderFreeMessages = async () => {
  ...
  const loadIdeaChat = async (ideaId: number) => {
  ...
  const loadOlderIdeaMessages = async () => {
  ...
  const handleFreeSend = async (message: string) => {
  ...
  const handleIdeaSend = async (message: string) => {
  ...
  const handleIdeaSelect = async (idea: ChatIdeaSummary) => {
    ...
    if (ideaError) {
      return (
        <div className="px-4 py-4">
          <p className="text-xs text-rose-600">{ideaError}</p>
          <button
            type="button"
            onClick={loadIdeas}
      ...
      <button
        ...
        className={`w-full px-4 py-3 text-left transition-all ${
          selectedIdeaId === idea.ideaId
            ? 'bg-white border-l-4 border-l-[#6366f1]'
            : 'hover:bg-white/60'
        }`}
      >
  ...
  const formatTokensRemaining = (tokens: number | null) => {
  ...
  return (
    <>
      ...
      <div className={`fixed bottom-6 right-6 z-50 flex w-full flex-col items-end gap-3 px-4 md:px-0 ${
            ...
            <div className="flex flex-1 min-h-0 overflow-hidden">
              ...
              >
                <div className="w-64 flex flex-col h-full">
                  ...
                  <div className="flex-1 overflow-y-auto bg-gray-50">{renderIdeasList()}</div>
                </div>
              </aside>

              <div className="flex min-w-0 flex-1 flex-col bg-white overflow-hidden">
                {/* Breadcrumb - Mostra a ideia selecionada */}
